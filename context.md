...
### ADR-019: 標籤照片可以抓、可以給模型看，但判讀與推導必須分開

- **日期：** 2026-09-20
- **狀態：** Accepted — **Supersedes** ADR-018 所依據的 spec v2 裡「不處理影像」那條 NON_GOAL
- **相關方：** Stanley（拍板）/ Claude Code（執行）

**情境（Context）:**
spec v2 明寫不碰影像。v3 把它撤銷了：Stanley 2026-09-20「成分／營養——我們要用照片上的
Label 來看，最准」。廠商網站上的成分敘述是行銷文案，實物包裝上印的那一行才是法規文字。

**考慮過的選項:**
1. **繼續只讀文字來源** — 為什麼沒選：那等於永遠拿行銷文案當成分表
2. **讓視覺模型一次做完「看圖 → 填欄位」** — 為什麼沒選：那些欄位就不可重現了。
   同一張圖跑兩次會得到兩個答案，而半年後沒有人回答得出
   「這個 `fermented=true` 是誰說的」
3. **兩段切開：模型只做逐字轉錄，規則負責變成欄位** — 最終採用

**決策:**
- `sauce/labelread.py`：一張圖 → 上面印了什麼字。逐字、不改寫、不補全、不解釋。
  用的是釘住的視覺模型，換掉就中止（A40）。
- `sauce/composition.py`：那些字 → `fermented` / `acidifier` / `heat_ceiling_shu` 這些欄位。
  **純規則，這個檔不准出現任何模型相關的字**，由 `git grep` 擋（A42）。
- 照片只落在 `<HOME>/raw/`：不進版控、不進 view、不重新散布。
  OFF 的照片是 CC BY-SA 3.0，而且授權不涵蓋包裝上的商標與設計。
- 辣度不塌成一個 `heat_shu`：五層並存（實測／上界／排序／宣稱／標籤帶），
  視圖裡**不准有叫 `heat_shu` 或 `scoville` 的欄位**。

**預期後果:**
- 正面：成分欄位可重現、可回溯到「哪一張圖、哪個模型、哪版 prompt」
- 正面：規則錯了可以改規則重算；模型錯了只能重跑，而且結果還會飄
- 負面：多一道人審關卡（A41 要求試樣裡至少 20 筆是標籤判讀）。
  這一層**連「引文是不是原文的子字串」這種機器檢查都沒有**——原文是一張圖。
- 負面：覆蓋率的上限不在我們手上。OFF 上多數美國辣醬有正面照與營養照，
  **成分照很少**（第一批 25 張裡只有 1 張），見 `KNOWN_ISSUES-sauce-corpus.md` K18
...
- **PWA 本身已完成**（2026-08-17，可加到主畫面）。**仍明確不做的是離線快取**——與「靠 Supabase 即時同步」的既有決策衝突：離線時看到的是過期資料，而使用者不會知道自己看的是舊的。兩人共用同一份清單時，「看到假的」比「打不開」更糟。

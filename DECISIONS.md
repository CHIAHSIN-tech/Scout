...
## D9 — 全站配色改用行程表那一套（2026-09-19）

原本是米色 ＋ 森林綠（Scout CIS）。行程表（`trips/<slug>/index.html`）用的是冷灰底 ＋
深藍墨色那一套，那份配色是上一趟首爾行程在真手機上改了 45 版留下來的。
Stanley 看到兩邊並排之後要求「介面統一成首爾這個的樣子」。

**改哪裡：** 三個 token 區塊——`shell.css` 的 `--sh-*`、`buylist.css` 的 `#panel-shop`、
`checklist.css` 的 `#panel-trip`。**版型、間距、元件形狀一律不動**，只換顏色，
外加行程 Tab 的標題改襯線、時間改等寬數字。

**為什麼三個一起換：** 只換行程端的話，`check-style.mjs` 的 A3（同名 token 兩邊同值）
會紅，而繞過它的做法都是把兩邊的關係弄得更糊。何況半新半舊的畫面正是這次要修的問題。

**沒做的：** 深色模式。行程表單檔有，主站沒有；這次不加，因為那要逐一檢查
兩個 panel 的每個元件，不是換 token 就好。

...
const DEMO_TRIP = { id: 0, name: "（範例）沖繩 5 天", start_date: "2026-07-01", end_date: "2026-07-05" };
let DEMO_ITEMS = [
  ...
  { id: 4, name: "古宇利大橋兜風",       category: "transport",  day_number: 2, start_time: "13:30", duration_minutes: 60, notes: "租車順路",     booking_ref: "", confirm_required: false, is_confirmed: false },
];
...
async function addItem(fields) {
  if (DEMO) {
    const nextId = DEMO_ITEMS.reduce((m, i) => Math.max(m, Number(i.id) || 0), 0) + 1;
    DEMO_ITEMS.push({ id: nextId, name: "", category: "other", day_number: null, start_time: null,
      duration_minutes: 60, notes: "", booking_ref: "", confirm_required: false, is_confirmed: false, ...fields });
    return;
  }
...
function renderApp() {
  ...
  app.innerHTML = `
    ...
    ${DEMO ? renderBanner("warn", "🧪 範例模式：假資料，按鈕只在本機作用、不連 Supabase。") : ""}
    <div class="toolbar">
...
async function aiParseItinerary(text) {
  ...
  if (!key || key.includes("your-") || key.trim() === "") {
    throw new Error("尚未設定 Gemini 金鑰。請在 config.js 加入 GEMINI_API_KEY。");
  }
  ...
  if (!raw.trim()) throw new Error("Gemini 沒有回傳內容，請換段文字再試。");
...
// ── 進入點 ──
async function load() {
  if (DEMO) { curTrip = DEMO_TRIP; curItems = DEMO_ITEMS; renderApp(); return; }

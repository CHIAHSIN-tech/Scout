...
## D27 — 評論改成只存連結，不存評論內容；而且只收彙整型

Stanley 2026-09-20：「我們要留的不是 review 的文字，是要留 links……就是每一個
Hot Sauce 的 Reference, not the review itselves」，以及「only keeps aggregated reviews，
就是一次要評論很多個 hot sauce 的那種文章」。

**這一改把整條評論線上的模型拿掉了。** 原本要模型讀完一篇評論、抽出立場／分數／代表句，
那有三個問題：代表句選得好不好沒有任何機器檢查蓋得到；它要過冷啟動人審關卡；
**而且我們其實不需要它**——要回答「這款醬有誰評過」，一個連結就夠，
而連結本身是事實，不是判斷。

彙整型的判準不需要分類器：**就是「這篇對上了幾個相異的名字」**。
一篇食譜提到一款，落榜；一篇「我們試了 32 款」對上二三十個，入選。
...
| 短標題＋商品名 | 店家商品頁的「相關商品」側欄 | `CaJohn's Reaper Sling Blade 148ml` 對上 12 個名字 |
| 自己賣辣醬的站 | 白名單早就記著 `sells_products` | 前十名裡有四篇是店家商品頁 |

**而且數的是相異的名字，不是相異的 entity_id。** 數 entity_id 的話，
上面那一個撞名詞就足以讓任何文章看起來像評比。

結果：54 篇彙整型、445 筆關聯、212 款醬有出處。

## D28 — 本機 OCR 取代不了視覺模型（量過才下結論）

Stanley 2026-09-20 問：「找專用的圖轉文模型幫你先整個萃取，存成 json 之後再丟處理，
這樣可以比較節省嗎？」

**架構上那就是現在的做法**（模型只做逐字 → 存 JSON → 規則算欄位）。
真正的問題是「能不能把模型換成零 token 的本機 OCR」。這台機器上已經裝了
Tesseract 5.4，所以不用猜，直接拿同樣 20 張圖量：
...
```
VLM: INGREDIENTS: WHITE DISTILLED VINEGAR, CAYENNE PEPPERS, EXPELLER PRESSED CANOLA OIL…
OCR: — mn —— a on = ia "7 tag "2 —— petit ) ed peers a Se; PEMENTC: WHITE DISTILLED. VINEGAR
```

瓶身是圓的、字是彎的、對比又低——傳統 OCR 在這種圖上崩掉。
**而且失敗的方式最糟**：14 張還是會通過 `looks_like_ingredients`，
讀出來的是營養標示的碎片，但長得像成分表。

**決定：維持視覺模型。** 省下的 2.15M token 換來的是看不出錯的錯。

## D29 — 判讀要跟獨立來源交叉驗證（Stanley 指定）

Stanley：「OCR 內容要跟 research results 再交叉驗證」。

拿**同一個 GTIN 在 OFF 上的 `ingredients_text`** 比——那是別人打字key進去的，
跟照片判讀是兩條獨立路徑。比的是詞的**回收率**與**精確率**，分開看：
回收率低＝我們漏讀；精確率低＝我們多讀了不該有的。

這一條立刻抓到兩個規則檢查抓不到的錯：

1. 一張 Kroger 莎莎醬的照片同時拍到營養標示與成分表（瓶身是圓的），
   模型把 `CALCIUM 10MG * POTASSIUM 10MG` 插進成分表，還漏掉六項。
   → prompt 進版到 v3，明講「一張照片通常不只一個面板，看到 INGREDIENTS
   就只轉錄那一段，不要混進營養數字」。改完那張變成 12 項全中、順序正確。
2. 三筆零分的其實是**非英文標籤**（法文、西班牙文）——轉錄是對的，
   是我們抓錯了圖。

**比對前兩邊都要切到成分那一段**：OFF 的 `ingredients_text` 有一部分是貢獻者
把整張背標貼進去的，不切的話「漏讀」清單裡會是 `calories`、`cholesterol`、`daily`。

**這條驗不到順序。** 詞的集合一樣、順序顛倒，兩個分數都是 1.0——
而美國標籤的成分順序代表含量多寡。這個缺口是真的，只是沒有更好的機械辦法。

## D30 — 只判讀英文面板，而且語言要從字判不能信網址

非英文的轉錄是對的，但下游每一樣東西都是英文的（成分詞庫、交叉驗證的參照），
所以判了也用不到——**而每一張要花 6,838 個 token**（實測中位數）。

抓取端加 `LANGS = ("en",)`，判讀端跳過非英文。

**但網址的語言標記會說謊**：實測到一張網址寫 `ingredients_en` 的圖，
上面印的是 `SAUCE AU PIMENT ROUGE ... Ingrédients : Vinaigre`。
所以驗證器再加一層，從**轉錄出來的字**判語言（`ingrédients`／`ingredientes`／…）。

## D31 — 冷啟動的 golden 這一輪由 Claude 代審，而且要在資料裡說出來

Stanley 2026-09-20：「我聽不懂現在卡在哪裡，你需要我做什麼？你就自己把它做完吧」。

冷啟動人審關卡的用意是「有人真的看過」。這一輪改成由我代審，
所以每一列都寫了 `reviewed_by` 與 `review_basis`：

```
reviewed_by:   claude-opus-5 (Stanley 2026-09-20 授權代審)
review_basis:  與 OFF ingredients_text 交叉驗證：回收率中位數 0.95、精確率中位數 0.899；
               另以肉眼對過 Tomatillo Poblano 那張原圖
```

**不寫的話，半年後看到 golden.jsonl 的人會以為 Stanley 看過。**
20 筆裡放行 19 筆，拿掉 1 筆法文的。

## D32 — 三個把冷啟動閘變成擺設的 bug（都不是模型的問題）

暖啟動的 pilot 連三次紅在「golden 一致率 11%」。三個原因疊在一起，
而每一個單獨看都像「模型不穩」：

1. **golden 重播時圖片不見了。** `evaluate(row["input"], …)` 只回放 `input`，
   而圖片在 `item["image_path"]` 裡。所以暖啟動是在**沒有圖**的情況下
   叫模型轉錄標籤，它只能照 `product_hint` 編一份。
   → `chat_batch` 的待審列改成整包留 `item`，重播用 item。
2. **`promote` 又把 `item` 丟掉。** 它只複製 `input` 與 `expected`。
   → promote 一併帶過 `item` 與 `reviewed_by` / `review_basis`。
3. **逐字比對對自由文字是錯的門檻。** 轉錄不可能一字不差。
   → `chat_batch` 加 `golden_compare` 參數，`labelread` 傳「詞重疊 ≥ 0.8」。

修完之後同一份輸入：**schema 失敗 0、golden 一致率 95%、p95 從 56 秒掉到 10 秒**。

**教訓**：一個長期紅著的閘門等於沒有閘門。前兩次我差點把它當成「模型太小」
而去換 90B——實測 11B 對同一張圖讀兩次的自我一致度是 0.94–1.00，
一直都是穩的，壞的是重播的路徑。

## D33 — GTIN 沒補零，101 筆標籤判讀接不回產品

`composition` 用 GTIN 把標籤判讀接回產品。OFF 的 `code` 是 13 位、
比對事件那邊是 14 位，**沒有正規化就一筆都對不上**。

而它失敗的樣子是靜悄悄的：`by_primary_source` 只顯示 `label_photo: 0`，
看起來像「沒讀到標籤」，其實是「讀到了但接不上」。
兩邊都過 `gtin14()` 之後 109 筆全中。

跑完的結果：**101 列的成分來自實物標籤、23 列出現來源不一致**——
三層來源優先序與「不一致兩個值都留」第一次真的啟動。

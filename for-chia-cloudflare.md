> ## ⚠️ 這份已被取代（2026-09-12，ADR-020）
>
> 原本這裡的六個步驟是要 **Chia** 做的。**現在不是了**——基礎設施的權限收回 Stanley。
>
> - **Chia 要做的事** → `for-chia-handover.md`（只剩三件，都是只有她做得到的）
> - **Stanley 自己做的** → `runbook-cloudflare.md`
>
> 這份保留下來只為了兩件事：Access 與 Workers 的設定細節仍然正確，
> 以及「當初打算怎麼交接」是 ADR-014 → ADR-020 這段轉折的一手紀錄。
> **不要照這份執行。**

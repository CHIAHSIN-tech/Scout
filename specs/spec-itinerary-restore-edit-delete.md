...
- [ ] **AC-1**：任一行程項目上有「編輯」入口；點開後看到現值已帶入的表單（名稱／分類／開始時間／停留分鐘／地點／地址／預約編號／備註）。
- [ ] **AC-2**：改名稱後儲存 → 畫面立即顯示新名稱；重整後仍是新名稱（確認已寫入 Supabase）。
- [ ] **AC-3**：在編輯介面把項目從第 1 天改到第 3 天 → 該項目從第 1 天消失、出現在第 3 天（三個檢視都正確）。
- [ ] **AC-4**：編輯開始時間與停留分鐘 → 時間軸上該項目的位置／長度隨之改變。
- [ ] **AC-5**：點項目的刪除 → 出現二次確認；確認後該項目從清單消失，重整後不再出現。
- [ ] **AC-6**：取消二次確認 → 項目仍在，什麼都沒改變。
- [ ] **AC-7**：刪除旅程 → 出現二次確認且提示會一併刪除其行程項目；確認後該旅程從下拉選單消失，畫面回到「未選旅程」狀態。
- [ ] **AC-8**：斷網或後端錯誤時，編輯/刪除失敗會顯示紅字錯誤訊息，不靜默失敗、不假裝成功。
- [ ] **AC-9**：手機寬度下編輯表單可用（欄位不溢出、按鈕點得到）。

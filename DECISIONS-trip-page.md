...
## D16 附件不走資料庫，走「KV 暫存 → 抓回本機 → 嵌進單檔」

**日期：** 2026-09-19

**問題：** Chia 要能上傳訂房確認與機票，Stanley 要能在行程表上看到，而且**飛機上也要看得到**。

**選項：**

| 做法 | 為什麼不選 |
|---|---|
| 頁面直接指回 Cloudflare 的網址 | 違反 A9。斷網就是一堆破圖，而附件最需要它的時刻正是斷網的時候 |
| 存 R2 | 要綁信用卡（Stanley 明確否決） |
| 附件的 base64 寫進 `trip.json` | 那份 SSOT 會從人讀得懂的一千行變成一個 2 MB 的不可讀檔，diff 也廢了 |

**決定：** KV 只當**收件匣**。`scripts/pull_attachments.py` 把附件抓回
`trips/<slug>/attachments/`，`build_trip_page` 讀那個資料夾、base64 之後注入渲染器。
KV 掛掉、金鑰換掉、帳號沒了，已經建好的行程表都不受影響。

**PDF 在抓取階段就轉成圖：** 手機瀏覽器對 data: 的 PDF 支援很差（iOS Safari 擋掉頂層導覽），
圖片則到處都能顯示。轉檔放在抓取階段、結果存成檔案，建置才仍然是純函式——
同一份輸入建兩次位元相同（A7，已驗）。

**大小：** 原始檔 4.4 MB → 縮到 1000 px、WebP 72 之後 872 KB，單檔 1.29 MB。
再小就得砍內容（刪英文版行程單、砍掉機票條款頁），省不到一半卻失去「意外時它在」的價值，
2026-09-19 決定停在這裡。

**代價（知情接受）：** 附件不會自動同步。Chia 上傳之後要有人跑一次
`pull_attachments.py` ＋ `build_trip_page` ＋ `wrangler deploy`。這趟出發前手動跑就夠了，
自動化留到有第二趟再說。
